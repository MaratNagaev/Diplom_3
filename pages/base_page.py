...
class BasePage:
    ...
    def wait_visibility_of_element(self, locator):
        WebDriverWait(self.driver, timeout=20).until(ec.visibility_of_element_located(locator))
    ...
    def wait_for_closing_of_element(self, locator):
        WebDriverWait(self.driver, timeout=20).until_not(ec.visibility_of_element_located(locator))

    @allure.step('Проверить кликабельность элемента')
    def check_element_is_clickable(self, locator):
        return WebDriverWait(self.driver, timeout=20).until(ec.element_to_be_clickable(locator))

    @allure.step('Подождать смену текста на элементе')
    def wait_for_element_to_change_text(self, locator, value):
        return WebDriverWait(self.driver, timeout=20).until_not(ec.text_to_be_present_in_element(locator, value))